...
def test_uninitialized_token(requests_mock):
    ...
    requests_mock.get('https://jsonplaceholder.typicode.com/', text='resp', status_code=200)
    resp = requests.get('https://jsonplaceholder.typicode.com/', auth=auth)
    resp = requests.get('https://jsonplaceholder.typicode.com/', auth=auth)
    
    assert resp.request.headers["Authorization"] == "Bearer hzefjzezelfjeaofe", "Token is wrong"
...
def test_unauthorized_token(requests_mock):
    ...
    with pytest.raises(requests.HTTPError):
        resp = requests.get('https://jsonplaceholder.typicode.com/', auth=SGConnectAuth("client_id", "client_secret", scopes=["admin"]))
